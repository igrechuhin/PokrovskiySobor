...
Object.extend(AC.VR.prototype, {
	...
	initialize: function(container, imagePath, totalFrames, options){
		...
		// dom
		this.container = _$(container).addClassName('vrcontainer');
		this.vr = _$(document.createElement('div')).addClassName('vr');
		...
		if (this.options.spinnable) {
			...
			for (var i=0; i<directions.length; i++) {
				var dir = directions[i],
				 	spinner = _$(document.createElement('div'));
	...
	unmakeInteractive: function(){
		...
		//
		// 	for (var i=0; i<directions.length; i++) {
		// 		var dir = directions[i], spinner = _$(document.createElement('div'));
	...
	onGrabStart: function(event){
		...
		this.grabbing = true;
		_$(document.body).addClassName('grabbing');

		_$(document).observe(this.getStr('mousemove'), this.bindGrabChange);
		_$(document).observe(this.getStr('mouseup'), this.bindGrabEnd);
		if (this.mobile) {
	...
	onGrabEnd: function(event){
		...
		this.grabbing = false;
		_$(document.body).removeClassName('grabbing');
		_$(document).stopObserving(this.getStr('mousemove'), this.bindGrabChange);
		_$(document).stopObserving(this.getStr('mouseup'), this.bindGrabEnd);
	...
	onSpinStart: function(event){
		this.spinning = true;
		_$(document.body).addClassName('spinning'+(this.spinDirection == -1 ? 'Left' : 'Right'));
		_$(document).observe(this.getStr('mousemove'), this.bindSpinChange);
		_$(document).observe(this.getStr('mouseup'), this.bindSpinEnd);
	...
	onSpinEnd: function(event){
		this.spinning = false;
		_$(document.body).removeClassName('spinning'+(this.spinDirection == -1 ? 'Left' : 'Right'));
		_$(document).stopObserving(this.getStr('mousemove'), this.bindSpinChange);
		_$(document).stopObserving(this.getStr('mouseup'), this.bindSpinEnd);
	...
	onFocus: function(event){
		this.focussed = true;
		_$(document).observe('keydown', this.bindKeyDown);
		_$(document).observe('keyup', this.bindKeyUp);
	},

	onBlur: function(event){
		this.focussed = false;
		this.vr.removeClassName('clickfocus');
		_$(document).stopObserving('keydown', this.bindKeyDown);
		_$(document).stopObserving('keyup', this.bindKeyDown);
	},
